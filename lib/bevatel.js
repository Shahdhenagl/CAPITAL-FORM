...
export async function sendTemplate({ to, name, kind, params }) {
  ...
  // On success the Developer API returns 201 with
  // {"message":"Message created successfully"}. Any non-2xx is thrown by devApi.
  const result = await devApi(cfg, `/messages`, {
    method: "POST",
    body: JSON.stringify(payload),
  });

  // Only treat an explicit failure status as an error (delivery status, when
  // present, is reported separately — a success body is NOT an error).
  const status = result?.data?.status || result?.status;
  if (status === "failed") {
    const extError =
      result?.data?.content_attributes?.external_error || "unknown error";
